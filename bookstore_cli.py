...
def print_menu() -> None:
    ...
    print("0. Search Books by Author")
...
def find_author(cursor:sqlite3.Cursor)-> None:
    keyword = input("Enter an author: ").strip()
    cursor.execute(
            "SELECT title, author FROM book WHERE author LIKE ?",
            (f"%{keyword}%",)
        )
